...
function myFunction(tendangnhap){
	if (confirm("Bạn có thật sự muốn xoá tài khoản này!") == true){
		window.location.replace("XoaTaiKhoan?id=" + tendangnhap);
	}
	else {
		window.location.replace("DSTaiKhoan");
	}
}
</script>
</head>
...
<div class="container">
    ...
    <div class="row">
        ...
        <div class="col-sm-9 col-md-9" style="float:right">
            <h3 style="color:red">Danh mục tài khoản</h3>
        <div class="table-responsive">
              ...
              <table class="table table-bordred table-striped">
                   ...
                   <c:forEach items="${DSTaiKhoan}" var="c">
                 <tr>
                    <td>${c.tendangnhap}</td>
                    <td>${c.matkhau}</td>
                    <td> ${c.hoten}</td>
                    ...
                    <td> ${c.nghenghiep}</td>
                    <td> ${c.quyenhan}</td>
                    <td><p data-placement="top" data-toggle="tooltip" title="Delete"><button onclick="myFunction('${c.tendangnhap}')" class="btn btn-danger btn-xs" data-title="Delete" data-toggle="modal" data-target="#delete" ><span class="glyphicon glyphicon-trash"></span></button></p></td>
                </tr>
                </c:forEach>
                        </table>
